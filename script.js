...
function loadTrack(track_index){
    ...
    curr_track.load();
    isMute ? curr_track.volume = 0 : setVolume();
...
function minutes(time) {
    const cur_minutes = Math.floor(time / 60);
    return cur_minutes < 10 ? "0" + cur_minutes : cur_minutes;
}
function seconds(time) {
    const cur_seconds = Math.floor(time - minutes(time) * 60);
    return cur_seconds < 10 ? "0" + cur_seconds : cur_seconds;
}
function setUpdate(){
    if(!isNaN(curr_track.duration)){
        seek_slider.value = curr_track.currentTime * (100 / curr_track.duration) || 0;

        curr_time.textContent = `${minutes(curr_track.currentTime)}:${seconds(curr_track.currentTime)}`;
        total_duration.textContent = `${minutes(curr_track.duration)}:${seconds(curr_track.duration)}`;
    }
}
